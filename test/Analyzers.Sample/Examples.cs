...
public sealed class OrderService : IOrderService
{
   ...
   public Task<Order> GetOrder(int id) =>
      Task.FromResult(new Order(id)); // PT0001 + PT0002

   public Task<Order> GetOrderByCodeAsync(CancellationToken token, string code) =>
      Task.FromResult(new Order(42)); // PT0002
}
...
public static class OrderEndpoints
{
   public static void MapOrderEndpoints(WebApplication app)
   {
      ...
      app.MapGet("/orders/{id:int}", async (int id, CancellationToken ct) =>
      {
      ...
      app.MapGet("/orders/no-ct/{id:int}", async (int id) =>
      {
         await Task.Delay(10);
         return Results.Ok(new Order(id));
      });
      ...
      app.MapGet("/orders/bad-ct/{id:int}", async (CancellationToken token, int id) =>
      {
